 ...
 * @성능 int[] temp: 29,756 kb, 179 ms, boolean [] visited: 23,700 kb, 135 ms
 * @분류
 * $조합,
 * @고려사항
 * BJ_14889_S3_스타트와링크와 동일한 문제
 * 선택되고 선택되지 않았는지를 표시할 꺼기 때문에 int [] temp 보다 boolean [] visited를 써보자.(중복 체크를 하는것은 아님. 상태만)
 */
public class SWEA_모의_4012_요리사 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int t = 1; t <= T; t++) {
            ...
            foodA = new int[N / 2];
            foodB = new int[N / 2];
            //combination(N / 2, foodA, 0);
            combination(N / 2, new boolean[N], 0);
    ...
    /**
     * 마치 부분집합처럼 포함과 비포함으로 생각하기
     * 
     * @param toChoose
     * @param visited
     * @param startIdx
     */
    static void combination(int toChoose, boolean[] visited, int startIdx) {
        if (toChoose == 0) {
            // System.out.println(Arrays.toString(visited));
            for (int i = 0, a = 0, b = 0; i < visited.length; i++) {
                if (visited[i]) {
                    foodA[a++] = i;
                } else {
                    foodB[b++] = i;
            ...
            int satisA = getSynergy(foodA);
            int satisB = getSynergy(foodB);
            A = Math.min(A, Math.abs(satisB - satisA));
            return;
        }

        // T,T,F,F와 F,F,T,T는 같으므로 반만 구하기
        for (int i = startIdx; i < N - toChoose; i++) {
            visited[i] = true;
            combination(toChoose - 1, visited, i + 1);
            // 선택된 요소를 뽑을 때는 요소들이 업데이트 되므로 되돌릴 필요가 없었지만 visited를 사용할 때는 reset 시켜줘야 함
            visited[i] = false;
        }
    }
    ...
    static int getSynergy(int[] comb) {
        ...
        for (int i = 0; i < comb.length; i++) {
            for (int j = 0; j < comb.length; j++) {
                satis += map[comb[i]][comb[j]];
            }
        }
        return satis;
    }

    static void combination(int toChoose, int[] temp, int start) {
        if (toChoose == 0) {
        ...
        for (int i = start; i < N; i++) {
            temp[toChoose - 1] = i;
            combination(toChoose - 1, temp, i + 1);
            // temp를 되돌려???
        }
    }
